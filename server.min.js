...
function processCommand(receivedMessage) {
  ...
  let messageArguments = fullCommand.match(_helpers_helpers__WEBPACK_IMPORTED_MODULE_2__["regexes"].ARGUMENTS)
...
async function handleGameUpload(messageArguments, receivedMessage){
  ...
  if (messageArguments[1].match(_helpers_helpers__WEBPACK_IMPORTED_MODULE_2__["regexes"].GDRIVE)){
    link = Object(_helpers_helpers__WEBPACK_IMPORTED_MODULE_2__["parseGDriveLink"])(messageArguments[1])
  } else if (messageArguments[1].match(_helpers_helpers__WEBPACK_IMPORTED_MODULE_2__["regexes"].DROPBOX)) {
    messageArguments[1].slice(-1) === "0" ? link = Object(_helpers_helpers__WEBPACK_IMPORTED_MODULE_2__["parseDropboxLink"])(messageArguments[1]) : link = messageArguments[1]
  } else {
    link = messageArguments[1]
  }

  if (!messageArguments[4].match(_helpers_helpers__WEBPACK_IMPORTED_MODULE_2__["regexes"].REGIONS)){
    return  receivedMessage.channel.send(
        `invalid arguments for upload command`
    )
  }

  if (link.match(_helpers_helpers__WEBPACK_IMPORTED_MODULE_2__["regexes"].URL)){
    const qr = Object(_helpers_helpers__WEBPACK_IMPORTED_MODULE_2__["createASCIIQrCode"])(link)
    ...
    collector.on('collect', message => {
      if (message.content === "yes") {
        ...
        try {
          Object(_db_db_qree__WEBPACK_IMPORTED_MODULE_4__["createQree"])(qr, link, name, messageArguments[3], messageArguments[4], messageArguments[5], receivedMessage.author.id)
        } catch (e) {
          console.log(e)
  ...
  try {
    ...
    if (rows.length === 0){
      ...
      rows.map(({qr_data, qr_link, name, platform, region , size}, index) => {
        embeds.push(new discord_js__WEBPACK_IMPORTED_MODULE_0__["MessageEmbed"]()
