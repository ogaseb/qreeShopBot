...
async function scrapChannelforQrCodes(messageArguments, receivedMessage) {
  ...
  Object(_helpers_helpers__WEBPACK_IMPORTED_MODULE_4__["limitlessFetchMessages"])(receivedMessage.channel)
   .then(async messages => {
     // console.log(data)
     ...
     messages.forEach(async item =>{
       if (!!item.attachments.size) {
         ...
         const metaInformation = item.content.match(_helpers_helpers__WEBPACK_IMPORTED_MODULE_4__["regexes"].SCRAPER_TITLE)
         ...
         const res = await node_fetch__WEBPACK_IMPORTED_MODULE_2___default()(`${item.attachments.values().next().value.proxyURL}`)
...
async function handleGameUpload(messageArguments, receivedMessage) {
  ...
  if (messageArguments[1].match(_helpers_helpers__WEBPACK_IMPORTED_MODULE_4__["regexes"].GDRIVE)) {
    link = Object(_helpers_helpers__WEBPACK_IMPORTED_MODULE_4__["parseGDriveLink"])(messageArguments[1]);
  } else if (messageArguments[1].match(_helpers_helpers__WEBPACK_IMPORTED_MODULE_4__["regexes"].DROPBOX)) {
    messageArguments[1].slice(-1) === "0"
      ? (link = Object(_helpers_helpers__WEBPACK_IMPORTED_MODULE_4__["parseDropboxLink"])(messageArguments[1]))
      : (link = messageArguments[1]);
  } else {
    link = messageArguments[1];
  }

  if (!messageArguments[4].match(_helpers_helpers__WEBPACK_IMPORTED_MODULE_4__["regexes"].REGIONS)) {
    return receivedMessage.channel.send(
      `invalid region argument for upload command`
    );
  }

  if (link.match(_helpers_helpers__WEBPACK_IMPORTED_MODULE_4__["regexes"].URL)) {
    const qr = Object(_helpers_helpers__WEBPACK_IMPORTED_MODULE_4__["createASCIIQrCode"])(link);
